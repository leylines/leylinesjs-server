...
module.exports = function (options) {
  function endpoint(path, router) {
    if (options.verbose) {
      console.log(
        "http://" + options.hostName + ":" + options.port + "/api/v1" + path,
        true
      );
    }
  ...
  var bypassUpstreamProxyHostsMap = (
  ...
  var esriTokenAuth = require("./controllers/esri-token-auth")(
    options.settings.esriTokenAuth
  );
  if (esriTokenAuth) {
  ...
  endpoint(
    "/proxyabledomains",
    require("./controllers/proxydomains")({
  ...
  app.use(
    "/init",
    require("./controllers/initfile")(initPaths, error404, options.configDir)
  );

  var feedbackService = require("./controllers/feedback")(
    options.settings.feedback
  );
  if (feedbackService) {
  ...
  if (options.settings && options.settings.singlePageRouting) {
    var singlePageRoutingService = require("./controllers/single-page-routing")(
      options,
      options.settings.singlePageRouting
    );
