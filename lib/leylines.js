...
exports.createLine = async function(req, res, pool) {
  ...
  try {
    czmlfile = await czml.createHeader(czmlfile);
    czmlfile = await postgis.pgGetMultiline(czmlfile, pool, maintable, group, linewidth, colorArray);
    czmlfile = await postgis.pgFindGisPOI(czmlfile, pool, group, distance, colorArray);
    return res.json(czmlfile);
  } catch (error) {
...
exports.exportLine = async function(req, res, pool) {
  ...
  try {
    kmlfile = await postgis.pgExportMultiline(kmlfile, pool, maintable, group, linewidth, colorArray);
