...
exports.createGrid = async function(req, res, pool) {
   ...
   let solids = JSON.parse(JSON.stringify(leylinesSolids[category][solid]));
...
exports.exportGrid = async function(req, res, pool) {
   ...
   let solids = JSON.parse(JSON.stringify(leylinesSolids[category][solid]));
   let points = await leylinesSolids.getPoints(latitude, longitude, bearing, form, solids['points'])

   var styles = {}
   styles['highlight'] = {};
   styles['highlight']['color'] = "#ff00d7ff";
   styles['highlight']['width'] = 5;
   styles['color'] = solids['color']
   styles['distance'] = solids['distance']

   var kmlName     = category.charAt(0).toUpperCase() + category.slice(1).toLowerCase() + " " + solid.charAt(0).toUpperCase() + solid.slice(1).toLowerCase();
   var kmlfile     = []
   var kmlHeader   = await kml.getHeader(kmlName);
   var kmlFooter   = await kml.getFooter();
   if (type == "lines") {
      var kmlStyleMap = await kml.getLineStyleMap(category + "-" + solid + "-" + type);
      var kmlStyle    = await kml.getLineStyle(category + "-" + solid + "-" + type, styles);
   }
   var kmlGeometry = await kml.getGeometry(solids, points, category, solid, type, latitude, longitude, bearing, form);
   kmlfile = await kmlHeader.concat(kmlStyleMap, kmlStyle, kmlGeometry, kmlFooter);
   var filename = category + "_" + form + "_" + solid + "_" + type + "_" + latitude + "_" + longitude + "_" + bearing + ".kml"
...
exports.createPOI = async function(req, res, pool) {
   ...
   var select = "SELECT " + maintable + ".id," + maintable + ".site as name," + maintable + ".description," + maintable + ".type," + maintable + ".sub_type," + maintable + ".data_owner," + maintable + ".links," + maintable + ".importance," + maintable + ".x_coord," + maintable + ".y_coord," + maintable + ".z_coord," + maintable + ".country" + " FROM "  + maintable  + " WHERE " + maintable + ".type = '" + req.params.type + "';";
   var scale = 0.2;
   var czmlfile = [];

   czmlfile = await czml.createHeader(czmlfile);
   czmlfile = await postgis.pgCreatePoint(czmlfile, pool, select, req.params.type, scale, "icons", "poi");
   return res.json(czmlfile);
...
exports.createMegalithic = async function(req, res, pool) {
   ...
   var select = "SELECT id, type, name, description, data_owner, x_coord, y_coord, icon, country FROM megalithic.sites";
...
exports.createSacredSites = async function(req, res, pool) {
   ...
   var select = "SELECT id, name, description, data_owner, x_coord, y_coord, country FROM sacredsites.sites;";
   var scale = 0.2;
   var czmlfile = [];
...
exports.createInterfaithMarySites = async function(req, res, pool) {

   var czml = require('./createCzml.js');
   var postgis = require('./postGIS');

   var select = "SELECT id, name, description, image, data_owner, x_coord, y_coord, country FROM interfaithmary.sites;";
   var scale = 0.2;
   var czmlfile = [];
...
exports.exportPOI = async function(req, res, pool) {
   ...
   var kmlfile = []; 
   var kmlHeader = await kml.getHeader("Points of Interest");
   var kmlFooter = await kml.getFooter();
   var kmlStyle  = await kml.getPointStyle(type);

   kmlfile = await postgis.pgExportPoint(kmlfile, pool, select, type);
   kmlfile = await kmlHeader.concat(kmlStyle, kmlfile, kmlFooter);
   var filename = req.params.type + ".kml";
   res.header('Content-Type', 'text/xml');
   res.attachment(filename);
   res.status(200).send(kmlfile.join('\n'));
   return;
...
exports.createLine = async function(req, res, pool) {
   ...
   var color = req.params.color.split(",").map((x) =>parseInt(x));
   ...
   try {
      czmlfile = await postgis.pgGetMultiline(czmlfile, pool, maintable, group, linewidth, color);
...
exports.exportLine = async function(req, res, pool) {
   ...
   styles['highlight']['width'] = 5;
   styles['color'] = color;
   styles['distance'] = linewidth;

   var kmlName     = group;
   var kmlfile     = []
   var kmlHeader   = await kml.getHeader(kmlName);
   var kmlFooter   = await kml.getFooter();
   var kmlStyleMap = await kml.getLineStyleMap(group);
   var kmlStyle    = await kml.getLineStyle(group, styles);

   var kmlfile = await postgis.pgExportMultiline(kmlfile, pool, maintable, group, linewidth, color);
   kmlfile = await kmlHeader.concat(kmlStyleMap, kmlStyle, kmlfile, kmlFooter);
   var filename = group + ".kml"
   res.header('Content-Type', 'text/xml');
   res.attachment(filename);
   res.status(200).send(kmlfile.join('\n'));
   return;
