...
exports.exportGrid = async function(req, res, pool) {
   ...
   if (type == "lines") {
      ...
      var kmlStyle    = await kml.getLineStyle(solids, category, solid, type, styles);
   }
...
exports.createInterfaithMarySites = async function(req, res, pool) {
   ...
   var select = "SELECT id, site, description, data_owner, x_coord, y_coord, country FROM interfaithmary.sites;";
   var scale = 0.2;
   var czmlfile = [];
...
//   }
   ...
   kmlfile = await postgis.pgExportPoint(kmlfile, pool, select, type);
   kmlfile = await kmlHeader.concat(kmlStyle, kmlfile, kmlFooter);
   var filename = req.params.group + ".kml";
   res.header('Content-Type', 'text/xml');
   res.attachment(filename);
   res.status(200).send(kmlfile.join('\n'));
   return;
...
exports.createMegalithic = async function(req, res, pool) {
   ...
   try {
      ...
      for (var i=0, tot=rows.length; i<tot; i++) {
         ...
         description += "<tr><td>Data Custodian</td><td></td><td><a href=\"https://www.megalithic.co.uk/article.php?sid=" + rows[i].sid + "\" target=\"_blank\">megalithic.co.uk</a></td></tr>";
         description += "</table>";
...
exports.createSacredSites = async function(req, res, pool) {
   ...
   var select = "SELECT id, site, description, x_coord, y_coord, country FROM sacredsites.sites;";
   ...
   try {
      ...
      for (var i=0, tot=rows.length; i<tot; i++) {
         ...
         description += "<tr><td>Country</td><td>" + rows[i].country + "</td></tr>";
         description += "</table>";
         var icon = "/images/icons/button_cyan.png";
         var scale = 0.2;
         czmlfile = czml.createPoint(rows[i].id, rows[i].site, description, icon, scale, Number(rows[i].x_coord), Number(rows[i].y_coord), 0.0, czmlfile);
      }
   } finally {
