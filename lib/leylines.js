...
exports.createUnesco = async function(req, res, pool) {
  const czml = require('./createCzml.js');
  const postgis = require('./postGIS');
  ...
  try {
    czmlfile = await czml.createHeader(czmlfile);
    czmlfile = await postgis.pgCreatePoint(czmlfile, pool, select, "button_green", scale, "icons", "site");
    return res.json(czmlfile);
  } catch (error) {
    console.error(error);
    return res.status(500).send('Internal Server Error');
...
exports.createStarCities = async function(req, res, pool) {
  const czml = require('./createCzml.js');
  const postgis = require('./postGIS');
...
exports.createDelaunay = async function(req, res, pool) {
  const czml = require('./createCzml');
  const postgis = require('./postGIS');
  var { schema, table, width, color, group } = req.params;
  ...
  try {
    czmlfile = await czml.createHeader(czmlfile);
    czmlfile = await postgis.pgGetDelaunay(czmlfile, pool, maintable, linewidth, colorArray, group);
    return res.json(czmlfile);
  } catch (error) {
...
exports.createVoronoi = async function(req, res, pool) {
  const czml = require('./createCzml');
  const postgis = require('./postGIS');
  var { schema, table, width, color, group } = req.params;
  ...
  try {
    czmlfile = await czml.createHeader(czmlfile);
    czmlfile = await postgis.pgGetVoronoi(czmlfile, pool, maintable, linewidth, colorArray, group);
    return res.json(czmlfile);
  } catch (error) {
    console.error('Error creating line:', error.message);
    return res.status(500).send('Internal Server Error');
