...
exports.createGrid = async function(req, res, pool) {
   ...
   if (form == "points") {
      ...
      try {
         czmlfile = await geometry.createCorridor(res, points, solids[type][form], solids['environment'][type][form]['distance'], solids['environment'][type][form]['color'], czmlfile);
      } catch (error) {
         console.log('caught', error.message);
      }
      try {
...
exports.createLine = async function(req, res, pool) {
	...
	await czml.createHeader(czmlfile);
        ...
        try {
           ...
           czmlfile = await geometry.createGisCorridor(res, multiline, linewidth, color, czmlfile);
        } catch (error) {
           console.log('caught', error.message);
        }
        try {
