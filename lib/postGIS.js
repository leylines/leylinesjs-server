...
exports.pgFindPOI = async function(res, pool, data, lines, distance, color, czmlfile, callback) {
   ...
   try {
      ...
      for (i=0, tot=rows.length; i<tot; i++) {
         ...
         czmlfile = czml.createPoint(rows[i].id, rows[i].site, description, icon, (rows[i].importance * 0.06), Number(rows[i].x_coord), Number(rows[i].y_coord), Number(rows[i].z_coord), czmlfile);
      }
