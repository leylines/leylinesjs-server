...
exports.pgGetDelaunay = async function(czmlfile, pool, maintable, linewidth, color, group) {
  const html = require('./createHTML');
  const geometry = require('./createGeometry');

  const select = `
    WITH edges AS (
    SELECT (ST_Dump(ST_DelaunayTriangles(ST_Collect(geom::geometry), 0.0, 1))).geom AS geom FROM ${maintable}` +
    (group ? ` WHERE "group" = $1)` : `)`) +
    ` SELECT ST_AsGeoJson(geom) as geom FROM edges`

  const client = await pool.connect();
  try {
    const { rows } = group ? await client.query(select, [group]) : await client.query(select);
    let id = 0;
    for (const { geom } of rows) {
      ...
      const featureDescription = html.createFeatureInfo({ id, id, id }, "line", "internal");
      czmlfile = await geometry.createGisCorridor(czmlfile, id, polyline, id, featureDescription, linewidth, color);
      id++;
    }
  } finally {
    client.release();
...
exports.pgGetVoronoi = async function(czmlfile, pool, maintable, linewidth, color, group) {
  const html = require('./createHTML');
  const geometry = require('./createGeometry');

  const select = `
    WITH edges AS (
    SELECT (ST_Dump(ST_VoronoiLines(ST_Collect(geom::geometry), 0.0))).geom AS geom FROM ${maintable}` +
    (group ? ` WHERE "group" = $1)` : `)`) +
    ` SELECT ST_AsGeoJson(geom) as geom FROM edges`
...
exports.pgGetMultiline = async function(czmlfile, pool, maintable, group, linewidth, color, type) {
  const html = require('./createHTML');
  const geometry = require('./createGeometry');
...
exports.pgExportMultiline = async function(kmlfile, pool, maintable, group, linewidth, color, type) {
  const html = require('./createHTML');
...
exports.pgFindGisPOI = async function(czmlfile, pool, group, distance, color, type) {
  ...
  try {
    ...
    let selectPoints = `
      ...
      ON "group" = $1` + 
      (type == '%' ? ` AND ("type" LIKE $3 OR "type" IS NULL)` : ` AND "type" = $3`) +
...
exports.pgCreatePoint = async function(czmlfile, pool, select, image, scale, dir, type) {
   ...
   var icon;
   if ((type == "poi") && (image != "Military") && (image != "unknown")) {
      icon  = "/images/" + dir + "/" + image.toLowerCase() + ".svg";
      scale = 0.06;
   } else {
      icon  = "/images/" + dir + "/" + image.toLowerCase() + ".png";
   }
