...
exports.pgFindGisPOI = async function(czmlfile, pool, group, distance, color) {
   ...
   try {
      ...
      for (var i=0, tot=rows.length; i<tot; i++) {
         ...
         var scale = 0.05;
         var description = html.createFeatureInfo(rows[i], "internal");
...
exports.pgExportPoint = async function(kmlfile, pool, select, type) {
   ...
   try {
      ...
      //});
      for (var i=0, tot=rows.length; i<tot; i++) {
         var description = html.createFeatureInfo(rows[i], "external");
         kmlfile = await kml.createPoint(rows[i].site, description, Number(rows[i].x_coord), Number(rows[i].y_coord), Number(rows[i].z_coord), type, kmlfile);
      }
      kmlfile.push("   </Folder>");
   } finally {
      client.release()
